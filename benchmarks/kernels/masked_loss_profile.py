...
def timed_step_loop(step, *, device: str, timer=None) -> tuple[float, list[float]]:
    ...
    for _ in range(STEPS):
        ...
        if timer is not None:
            # After the boundary synchronize: reading an event before its work
            # completes would force a sync per scope.
            timer.collect()
...
def profile_cell_for(args, *, vocab_size: int, layers: int, batch_size: int):
    ...
    def measure(arm: str, seed: int) -> None:
        ...
        if timer is None:
            warmup_seconds, timed = timed_step_loop(run, device=args.device)
        else:
    ...
    residual = model_forward - per_step("attention") - per_step("lm_head_projection")
    if residual < 0:
