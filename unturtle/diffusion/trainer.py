    ...
    sparse_lm_head: bool = field(
        ...
        metadata={
            "help": (
                ...
                "to the dense path.\n"
                "Measured end-to-end through this trainer "
                "(benchmarks/sparse_lm_head_training.py, B=2 L=512 H=512): "
                "step time -62%% at 128K vocab / 15%% masking, -26%% at 50%%, "
                "-4%% at 75%%; and -33%% / -13%% / +1%% at 32K vocab.\n"
                "Activation memory is the axis that turns: -37%% to -41%% at "
                "15%% masking, roughly neutral at the ~50%% ratio MDLM "
                "training averages (+0.4%% to +1.7%%), and worse above it "
                "(+34%% to +46%% at 75%%).\n"
                "So the step-time win is real and large for big vocabularies, "
                "while memory only improves at low mask ratios.  Defaults off; "
                "enable for large vocabularies at low mask ratios.  Note LoRA "
                "is markedly worse above ~15%% masking (+47%% activation "
                "memory at 50%%, +113%% at 75%%) because the frozen backbone's "
                "activations already dominate.\n"
                "Requires a backbone declaring 'sparse_output_projection' "
                "(Tiny-A2D today; LLaDA and MDLM-DiT are structurally "
                "excluded); raises at construction otherwise rather than "
