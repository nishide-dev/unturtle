...
if "default" not in _ROPE_INIT_FUNCTIONS:
    def _rope_default_init(config, device, **kwargs):
        import torch
        base = getattr(config, "rope_theta", 10000.0) if config is not None else kwargs.get("base", 10000.0)
        dim = (
            getattr(config, "head_dim", None)
            or (config.hidden_size // config.num_attention_heads if config is not None else kwargs.get("dim"))
        )
        max_position_embeddings = (
            getattr(config, "max_position_embeddings", 2048) if config is not None else kwargs.get("max_position_embeddings", 2048)
        )
